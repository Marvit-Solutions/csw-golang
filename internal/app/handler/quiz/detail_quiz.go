...
func (h *handler) DetailQuiz(ctx *gin.Context) {
	var req request.ParamDetailQuiz
	...
	detailQuiz, err := h.u.DetailQuiz(req)
	if err != nil {
		helper.NewErrorResponse(ctx, http.StatusUnprocessableEntity, http.StatusText(http.StatusUnprocessableEntity), err.Error())
		return
	}

	helper.NewSuccessResponseNonPaged(ctx, http.StatusOK, http.StatusText(http.StatusOK), detailQuiz)
}
