package seed

import (
	ds "csw-golang/internal/domain/entity/datastruct"
	"time"
)

func CreateSubjectTestTypeQuizzes() []*ds.SubjectTestTypeQuizzes {
	subjectTestTypeQuizzes := []*ds.SubjectTestTypeQuizzes{
		{
			ID:          "8c88d1e5-eb9f-4d4d-a94e-94be94209cb0",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "58ab4e13-e016-4c42-8123-74b220db465a",
			TestType:    "Pretest",
			Title:       "Pretest Pancasila",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Selesai",
			Attempt:     1,
		},
		{
			ID:          "8049dc26-01c1-426a-803d-570b799d3810",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "58ab4e13-e016-4c42-8123-74b220db465a",
			TestType:    "Posttest ",
			Title:       "Posttest Pancasila",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Dibuka",
			Attempt:     1,
		},
		{
			ID:          "b1d5958d-96fc-4b85-8f83-78e4eb8ffbbc",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "b8aa7572-a9b6-40e4-9ef5-06cf13f79e49",
			TestType:    "Pretest",
			Title:       "Pretest Konstitusi yang berlaku di Indonesia",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Dibuka",
			Attempt:     1,
		},
		{
			ID:          "2b703858-1fbd-49ad-8574-4ff5962dc92c",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "b8aa7572-a9b6-40e4-9ef5-06cf13f79e49",
			TestType:    "Posttest ",
			Title:       "Posttest Konstitusi yang berlaku di Indonesia",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Dibuka",
			Attempt:     1,
		},
		{
			ID:          "68acf1ca-55d2-4666-9f7d-4ed32eac62d8",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "94905d6b-fab1-43f5-ad1e-573b1feea26d",
			TestType:    "Pretest",
			Title:       "Pretest NKRI & Bhinneka Tunggal Ika",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Dibuka",
			Attempt:     1,
		},
		{
			ID:          "aba04800-d0c7-46de-ba87-32809d651d02",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "94905d6b-fab1-43f5-ad1e-573b1feea26d",
			TestType:    "Posttest ",
			Title:       "Posttest NKRI & Bhinneka Tunggal Ika",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Dibuka",
			Attempt:     1,
		},
		// Module Test Pancasila
		{
			ID:          "f8259b82-41ca-4d58-90e7-c2be3f4c235f",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "58ab4e13-e016-4c42-8123-74b220db465a",
			TestType:    "Module Test",
			Title:       "Module Test Pancasila",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Selesai",
			Attempt:     1,
		},
		{
			ID:          "f8259b82-41ca-4d58-90e7-c2be3f4c235q",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "b8aa7572-a9b6-40e4-9ef5-06cf13f79e49",
			TestType:    "Module Test",
			Title:       "Module Test Konstitusi yang berlaku di Indonesia",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Selesai",
			Attempt:     1,
		},
		{
			ID:          "94905d6b-fab1-43f5-ad1e-573b1feea26s",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "94905d6b-fab1-43f5-ad1e-573b1feea26d",
			TestType:    "Module Test",
			Title:       "Module Test NKRI & Bhinneka Tunggal Ika",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Selesai",
			Attempt:     1,
		},
		{
			ID:          "58cdaedb-c3a0-47b6-96c3-bbb3bad537c7",
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			SubjectID:   "58cdaedb-c3a0-47b6-96c3-bbb3bad537c7",
			TestType:    "Module Test",
			Title:       "Module Test Demokrasi Indonesia",
			MeetingDate: "Jumat, 30 September 2022, 07:00",
			Open:        "Kamis, 29 September 2022, 07:00",
			Description: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book...",
			Time:        15,
			Point:       100,
			Status:      "Selesai",
			Attempt:     1,
		},
	}

	return subjectTestTypeQuizzes
}
